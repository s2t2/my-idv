...
def test_text_recognition():
    ...
    image_text = "NEW YORK STATE\nCommission\nof Motor Vehicles\nDRIVER LICENSE\nID: 000 000 000\nCLASS DM\nSAMPLE,SALLY\n1010 ANYPLACE ST\nYOURCITY NY 12121\nDOB: 07-18-83\nSEX: F EYES: BL HT: 5-09\nE: NONE\nR: NONE\nISSUED: 07-18-05 EXPIRES: 07-18-13\nSety Somple\nCELSIOR\n83145522\n"

    #response = recognize_text(client, image_filepath)
    #assert isinstance(response, AnnotateImageResponse)
    #assert response.text_annotations[0].locale == "en"
    #assert response.text_annotations[0].description == expected_text

    assert recognize_text(client, image_filepath) == image_text
